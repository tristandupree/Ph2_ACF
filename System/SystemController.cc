	...
	void SystemController::initializeHw(std::string pFilename)
	{
		...
		for(pugi::xml_node ns = doc.child("Shelf");ns;ns=ns.next_sibling())
		{
			...
			for(pugi::xml_node nb = ns.child("BeBoard");nb;nb=nb.next_sibling())
			{
				...
				{
					cBeBoard.setReg(std::string(nr.attribute("name").value()),atoi( 
nr.first_child().value()));
				}
				...
				for(pugi::xml_node nm = nb.child("Module");nm;nm=nm.next_sibling())
				{
					...
					Module 
cModule(cShelveId,cBeId,nm.attribute("FMCId").as_int(),nm.attribute("FeId").as_int(),cModuleId);
					fShelveVec[cNShelve]->getBoard(cBeId)->addModule(cModule);

					for(pugi::xml_node nc = nm.child("CBC");nc;nc=nc.next_sibling())
					{
						Cbc 
cCbc(cShelveId,cBeId,nm.attribute("FMCId").as_int(),nm.attribute("FeId").as_int(),nc.attribute("Id").as_int(),nc.attribute("configfile").value());	
						for(pugi::xml_node ng = 
nm.child("Global_CBC_Register");ng!=nm.child("CBC");ng=ng.next_sibling())
						{		
							cCbc.setReg(std::string(ng.attribute("name").value()), 
atoi(ng.first_child().value()));
						}
						for(pugi::xml_node ngr = nc.child("Register");ngr;ngr=ngr.next_sibling())
						{				
							cCbc.setReg(std::string(ngr.attribute("name").value()), 
atoi(ngr.first_child().value()));
						}				
